document.addEventListener("DOMContentLoaded", () => {
    ...
    const loadData = () => {
        ...
        document.getElementById("info-name").innerHTML = `${name} <i class="fa fa-pencil edit-icon"></i>`;
        document.getElementById("info-email").innerHTML = `${email} <i class="fa fa-pencil edit-icon"></i>`;
        document.getElementById("info-phone").innerHTML = `${phone} <i class="fa fa-pencil edit-icon"></i>`;
        document.getElementById("info-city").innerHTML = `${city} <i class="fa fa-pencil edit-icon"></i>`;
        document.getElementById("info-intent").innerHTML = `${intent} <i class="fa fa-pencil edit-icon"></i>`;
        document.getElementById("info-gender").innerHTML = `${gender} <i class="fa fa-pencil edit-icon"></i>`;
        document.getElementById("info-breed").innerHTML = `${breed} <i class="fa fa-pencil edit-icon"></i>`;
        document.getElementById("info-age").innerHTML = `${age} <i class="fa fa-pencil edit-icon"></i>`;
    ...
    const enableEdit = (span) => {
        ...
        const inputType = span.id === "info-intent" || span.id === "info-gender" || span.id === "info-age" ? "select" : "input";
        ...
        if (inputType === "select") {
            const options = span.id === "info-intent"
                ...
                : span.id === "info-gender"
                ? ["Fêmea", "Macho", "Ambos"]
                : ["0 a 2 anos", "2 a 5 anos", "5 a 8 anos", "+ de 8 anos", "Sem preferência"];
            ...
            if (span.id === "info-phone") {
                input.addEventListener("input", (event) => {
                    ...
                    event.target.value = phoneValue.replace(/(\d{2})(\d{5})(\d{4})/, "($1) $2-$3");
                });
            }

            input.addEventListener("blur", () => {
                span.innerHTML = `${input.value || currentValue} <i class="fa fa-pencil edit-icon"></i>`;
